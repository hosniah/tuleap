 ...
 * @author gstorchi
 */
class SystemEvent_HG_REPO_CREATE extends SystemEvent {
    ...
    public function process() {
        ...
        try {
            ...
            if ( $sys_allow_restricted_users == 1) {
                $repository->setAccess( HgRepository::PRIVATE_ACCESS );
            } else {
                $repository->setAccess( HgRepository::PUBLIC_ACCESS );
            }
            $user = null;
            if ( !empty($userId) ) {
        ...
        catch (HgDriverException $e) {
            $this->error( $e->getMessage() );
            return false;
        }
        catch (HgBackendException $e) {
            $this->error( $e->getMessage() );
            return false;
        }
        catch (Exception $e) {
